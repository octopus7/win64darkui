...
class AppWindow {
 ...
 private:
  ...
  {
    MessageBoxW(hwnd_,
                L"win64darkui\n\nBlender-inspired Windows native UI shell.\nCurrent stage: custom regions, menu bar, fullscreen toggle, placeholder editor UI.",
                L"About win64darkui",
  ...
  void HandleMenuCommand(const UINT command)
  {
    switch (command) {
      ...
      case kMenuEditPreferences:
        ShowPlaceholderPopup(L"Edit > Preferences", L"Preferences editor is not implemented yet.");
        break;
  ...
  void ShowTopMenu(const int index)
  {
    const RegionModel *region = TopbarRegion();
    if (!region) {
      return;
    ...
    const TopMenuLayout layout = BuildTopMenuLayout(*region);
    if (index < 0 || index >= static_cast<int>(layout.rects.size())) {
      return;
    }
    ...
    switch (index) {
      case 0:
        AppendMenuW(menu, MF_STRING, kMenuFileNew, L"New");
        AppendMenuW(menu, MF_STRING, kMenuFileOpen, L"Open...");
        AppendMenuW(menu, MF_STRING, kMenuFileSaveAs, L"Save As...");
        AppendMenuW(menu, MF_SEPARATOR, 0, nullptr);
        AppendMenuW(menu, MF_STRING, kMenuFileExit, L"Exit");
        break;
      case 1:
        AppendMenuW(menu, MF_STRING, kMenuEditUndo, L"Undo");
        AppendMenuW(menu, MF_STRING, kMenuEditRedo, L"Redo");
        AppendMenuW(menu, MF_SEPARATOR, 0, nullptr);
        AppendMenuW(menu, MF_STRING, kMenuEditPreferences, L"Preferences...");
        break;
      case 2:
        AppendMenuW(menu, MF_STRING, kMenuRenderImage, L"Render Image");
        AppendMenuW(menu, MF_STRING, kMenuRenderAnimation, L"Render Animation");
        AppendMenuW(menu, MF_SEPARATOR, 0, nullptr);
        AppendMenuW(menu, MF_STRING, kMenuRenderView, L"View Render");
        break;
      case 3:
        AppendMenuW(menu,
                    MF_STRING | (is_fullscreen_ ? MF_CHECKED : MF_UNCHECKED),
                    kMenuWindowToggleFullscreen,
                    L"Toggle Window Fullscreen");
        AppendMenuW(menu, MF_SEPARATOR, 0, nullptr);
        AppendMenuW(menu, MF_STRING, kMenuWindowScreenshot, L"Save Screenshot...");
        break;
      case 4:
        AppendMenuW(menu, MF_STRING, kMenuHelpManual, L"Online Manual");
        AppendMenuW(menu, MF_SEPARATOR, 0, nullptr);
        AppendMenuW(menu, MF_STRING, kMenuHelpAbout, L"About");
        break;
    }

    POINT anchor{
        static_cast<LONG>(layout.rects[index].left),
        static_cast<LONG>(layout.rects[index].bottom - 1.0f),
    };
    ...
    const UINT command = TrackPopupMenuEx(
        menu, TPM_LEFTALIGN | TPM_TOPALIGN | TPM_RETURNCMD | TPM_NONOTIFY, anchor.x, anchor.y, hwnd_, nullptr);
